# Original hardening audit (2026-05-06)

This is the snapshot audit produced when the hardening pass began. The
current status doc lives at `../hardening-status.md`. Many "open" items
listed below have since closed — see the current doc for citations.
...
§13–15 release-gate items. quic-zig is a Zig-first IETF QUIC v1 transport built
...
- Long-header structural peek before any decryption work:
  ...
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:1868–1895`.
- Inbound payload bound enforced at 4 KiB before frame parsing:
  ...
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:185–187`,
  `5995`, `6131`); `Connection.handle` rejects oversized datagrams with
  PROTOCOL_VIOLATION
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:5565–5570`).
- Per-source rate limiting on Initials, gated before any TLS or Connection
  allocation:
  `Server.acceptSourceRate`
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:958–973`,
  config flag `max_initials_per_source_per_window`).
- Slot-table cap before slot allocation:
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:949–953`.
...
- **No 1200-byte minimum check on Initial-bearing UDP datagrams.**
  ...
  datagrams smaller than 1200 bytes; quic-zig's Server.feed accepts any
  long-header datagram of size ≥ 6 bytes and the per-Connection
  `handleInitial` only checks DCID-extraction lengths
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:6109–6122`).
...
- Per-path bytes_received / bytes_sent accounting and the 3× cap:
  ...
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/path.zig:121–185`).
- Anti-amp gating applied to Initial+Handshake+1-RTT outbound, not just
  ...
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:4770–4783`,
...
- Per-path migration anti-amp reset (zeroes counters, re-credits triggering
  ...
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/path.zig:364–388`).
- Tests cover unvalidated server, validated server, migration boundary:
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:11270`,
  `11294`, `11336`, `11416`.
...
- HMAC-SHA256 over QUIC version + issued/expires timestamps + length-prefixed
  ...
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/retry_token.zig:1–166`.
- Constant-time tag comparison (`std.crypto.timing_safe.eql`) at line 132.
- Domain separator string `"quic-zig retry token v1"` at line 34.
...
- Server-side Retry gate (`Server.applyRetryGate`,
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:1563–1619`) refuses
...
- The Retry token format does not include encryption — only authentication.
  ...
  address/timestamp/routing/deployment metadata; quic-zig's token does carry
...
- Server-issued SCIDs are random and CSPRNG-sourced (`Server.random`
  ...
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:794–797`,
...
- CIDs are issued and retired through `provideConnectionId` /
  ...
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:1370–1414`) keeps
...
- Tokens are 16 bytes per RFC, supplied by the embedder via
  ...
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:348–352`);
  quic-zig does **not** mint them.
- **No HMAC-from-secret derivation helper.** The hardening guide requires
  ...
  reused; quic-zig accepts whatever the embedder supplies and has no policy
...
- Inbound stateless-reset detection is implemented and matches against
  ...
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:6427–6433`.

### §4.6 Transport parameters — IMPLEMENTED

- Duplicate detection (linear scan) before storing each parameter:
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/tls/transport_params.zig:233–237`,
...
- Connection-side validation also enforces:
  - peer `max_udp_payload_size >= 1200`
    (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:3709–3712`),
  - client must not send server-only params
    (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:3731–3744`),
  - `original_destination_connection_id` echo binding
    (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:4225–4239`),
  - peer flow-control maxima clamped to local limits
    (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:3713–3717`).
- Local params self-check: `max_udp_payload_size >= 1200`
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:1178`).
- Unknown ids ignored after their length is validated
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/tls/transport_params.zig:372`).
...
- ACK frame parses use checked varints
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/frame/decode.zig:107–150`).
- Range-iteration arithmetic is underflow-checked
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/frame/ack_range.zig:63–90`).
- Connection-side validation rejects ACKs claiming PNs not yet sent — this
  is the Cloudflare-CVE class:
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:7088–7091`
...
- Iteration walks the bounded `SentPacketTracker` rather than the peer's
  ...
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:7100–7110`
...
- Local AckTracker bounded at 255 disjoint intervals
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/ack_tracker.zig:22`).
- Outbound ACK frame builder respects byte budgets and a configurable
  ...
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/ack_tracker.zig:218–256`).

What's missing:

- **No explicit cap on incoming `range_count`.** The decoder loops
  `while (i < range_count.value)`
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/frame/decode.zig:120`,
...
- Path validation state machine (RFC 9000 §8.2):
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/path_validator.zig`.
- Random PATH_CHALLENGE token from BoringSSL CSPRNG:
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:3897–3902`.
...
- Per-path PATH_RESPONSE handling: `recordPathResponse` clears queued
  ...
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:3936–3950`).
- Migration rollback on validation failure:
  `PathState.rollbackFailedMigration`
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/path.zig:391–404`).
- Embedder migration policy hook (`migration_callback`,
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:1366`).

What's missing or weak:

- **No explicit `handshakeDone()` gate before accepting a peer-driven
  migration.** `recordAuthenticatedDatagramAddress`
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:3977–4014`)
...
- All cryptography goes through `boringssl-zig` 0.5.0 pinned via
  `build.zig.zon`
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/build.zig.zon:7–9`,
  `/Users/nullstyle/prj/ai-workspace/quic-zig/build.zig:7–18`).
- AEAD, HMAC, hash, RNG all wrap BoringSSL primitives (see imports in
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/retry_token.zig:9`,
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/wire/protection.zig`,
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/wire/long_packet.zig`).
- TLS 1.3 only (`min_version` and `max_version` both
  `boringssl.raw.TLS1_3_VERSION`):
  `/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:780–781`.
- quic-zig does not implement its own AEAD, HKDF, or signature primitives.

### §5.2 0-RTT default — NOT IMPLEMENTED

- **`Server.init` enables 0-RTT by default**: `early_data_enabled = true`
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:783`,
  `1198`). Same for the convenience `Client`
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/client.zig:194`,
  `226`).
...
- The early-data context digest covering ALPN + replay-relevant transport
  ...
  (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/tls/early_data_context.zig`).
...
- No application-side gate constraining early-data requests to "idempotent
  only"; quic-zig surfaces `EarlyDataStatus` so the embedder can decide, but
...
(`/Users/nullstyle/prj/ai-workspace/quic-zig/src/tls/early_data_context.zig:126`)
and ALPN string-handling docs. The README and CHANGELOG describe quic-zig as
...
(`/Users/nullstyle/prj/ai-workspace/quic-zig/src/tls/early_data_context.zig:34`).
...
1. Network parser code contains `catch unreachable`, unsafe optional
   ...
   (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/wire/varint.zig:65`,
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/wire/short_packet.zig:377`,
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/wire/long_packet.zig:609`,
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/path.zig:539`,
   `545`, `627`,
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/sent_packets.zig:263`,
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/retry_token.zig:101`).
...
3. Any peer-controlled length can allocate before limit validation. —
   **PASS.** Every reassembly path checks the configured limit before
   accepting bytes:
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:6878–6879`
   (CRYPTO),
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:6821–6826`
   (DATAGRAM),
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:3173–3177`
...
7. Unknown frames or settings can accumulate without limit. — **PASS for
   ...
   (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/frame/decode.zig:103`),
...
10. Retry tokens expose plaintext server metadata. — **PASS.** Retry
    token format
    (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/retry_token.zig:90–104`)
...
11. Connection IDs expose shard/region/tenant/timestamp. — **PASS** by
    ...
    `/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:1285`). An
    embedder swapping in router-encoded CIDs is on their own.

12. Stateless reset tokens are reused. — **N/A**: quic-zig does not mint
    reset tokens (the embedder supplies them via
    `ConnectionIdProvision`,
    `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:348–352`).
    No quic-zig-side default-safe HMAC helper; flag this as a reasonable
    next addition.
...
14. CONNECTION_CLOSE includes detailed parser errors by default. —
    **FAIL.** Internal reason strings are sent on the wire as-is; e.g.
    ...
    `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig` and the
    encoder at
    `/Users/nullstyle/prj/ai-workspace/quic-zig/src/frame/encode.zig:338–341`.
...
15. Logs include cookies/Authorization/Retry tokens/etc. — **PASS** at
    the transport level: quic-zig has no HTTP layer, the `LogEvent` enum
    in `server.zig` exposes only addresses and counters, and qlog
    callbacks are opt-in (null by default,
    `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:936`).
...
17. Flow-control stalls cause unlimited buffering. — **PASS.** Bounded
    ...
    (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:199`),
...
19. Fuzzing finds any panic, leak, or unbounded growth. — **N/A**.
    Fuzz-smoke tests at
    `/Users/nullstyle/prj/ai-workspace/quic-zig/tests/fuzz_smoke.zig` are
...
5. Interop testing passes against multiple independent implementations —
   **yes.** `INTEROP_STATUS.md` documents quic-go / quiche / picoquic /
   ...
   `/Users/nullstyle/prj/ai-workspace/quic-zig/interop/` exists.
...
1. **0-RTT is enabled by default with no anti-replay** — §5.2.
   ...
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:783` and
...
2. **No 1200-byte minimum on Initial-bearing UDP datagrams** — §4.1.
   ...
   Fix: at `Server.feed`
   (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/server.zig:944–948`),
   reject `isInitialLongHeader(bytes)` paths where
   `bytes.len < min_quic_udp_payload_size`. The constant already
   exists at
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:189`.
...
3. **CONNECTION_CLOSE leaks internal parser reason strings** — §9.1
   ...
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:close()`
   call sites (~80) plus the encoder
   (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/frame/encode.zig:338`).
...
4. **Migration accepted on authenticated Initial/Handshake datagrams,
   ...
   (`/Users/nullstyle/prj/ai-workspace/quic-zig/src/conn/state.zig:3977`),
...
5. **No explicit cap on ACK frame `range_count`** — §4.7. Risk: bounded
   ...
   memory blows up.
   Fix: at
   `/Users/nullstyle/prj/ai-workspace/quic-zig/src/frame/decode.zig:113–125`
...
- No quic-zig-side helper for HMAC-derived stateless-reset tokens (§4.5)
